...
def _device_dtype_check_for_fused(p: torch.Tensor, cuda_unsupported: bool = False) -> None:
...
class SGD(Optimizer):  # noqa: D101
    def __init__(
        ...
        *,
        ...
        for group in self.param_groups:
            group["total_grad"] = [None for _ in range(len(group["params"]))]
    ...
    @_use_grad_for_differentiable
    def step(self, closure=None, momentum_update=False):
        ...
        for group in self.param_groups:
            ...
            sgd(
                ...
                delay_factor=getattr(self, "delay_factor", None),
                buffer_size=getattr(self, "buffer_size", None),
                total_grad=group["total_grad"],
...
def sgd(
    ...
    *,
    ...
    delay_factor: float,
    buffer_size: int,
    total_grad: List[Tensor],
    momentum_update: bool,
):
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if total_grad[i] is None:
            total_grad[i] = grad.detach().clone()
        else:
            total_grad[i] = total_grad[i].to(grad.device)
            total_grad[i] = total_grad[i] + grad
        ...
        if momentum != 0:
            ...
            if buf is None:
                ...
                buf.mul_(momentum).add_(total_grad[i] / buffer_size, alpha=1 - dampening)
            # else:
            #     buf.mul_(momentum).add_(grad, alpha=1 - dampening)

            if nesterov:
                scale = (1 - delay_factor * buffer_size + delay_factor) if momentum_update else delay_factor
                grad = grad.div(buffer_size).add(buf, alpha=momentum * scale)
        ...
        # with torch.no_grad:
        total_grad[i] = total_grad[i].to("cpu")
