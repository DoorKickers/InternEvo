...
class MasterServiceHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/global_status':
        ...
        elif parsed_path.path == '/alive_ps':
            ...
            self.end_headers()
            self.wfile.write(response_bytes)
        elif parsed_path.path == '/alive_group':
            result = dict(alive_group=self.state_manager.get_alive_group())
            response = result
            response_bytes = json.dumps(response).encode('utf-8') 
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
            ...
            self.end_headers()
            self.wfile.write(response_bytes)
        elif parsed_path.path == '/group_status':
